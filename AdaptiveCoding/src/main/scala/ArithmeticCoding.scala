...
class ArithmeticCoding(charTable: Map[String, Int], charNumber: Int, text:String, conditional:Boolean) {
  private var total = charTable.size
  private var weightTable: Map[String, Int] = charTable
  ...
  private def getRawBySegment(segm:Segment): Raw = {
    ...
    val charProbability: Map[String, Double] = weightTable.map((d)=>(d._1,(segm._2-segm._1)*d._2.toDouble/total))
  ...
  private def increment(char:String) = {
    weightTable = weightTable.updated(char,weightTable(char)+1)
    total+=1
  }
